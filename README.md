...
<img width="2848" height="1504" alt="og-image" src="https://github.com/user-attachments/assets/24a12942-b35c-43fc-bd73-8994f15dbb2c" />

旅行の立て替えをスマートに精算するWebアプリです。

**デモ：** [https://main.d23e9yi3475pdz.amplifyapp.com](https://main.d23e9yi3475pdz.amplifyapp.com)
...
* クライアントからの `userId` を直接信頼せず、Cognito の JWT から取得した値を使用
...
環境構築時にご自身のAWSアカウントのリソースIDに置き換えてください。

| リソース | 値 |
| :--- | :--- |
| **AWS Region** | `ap-northeast-1` |
| **Cognito User Pool** | `<YOUR_COGNITO_USER_POOL_ID>` |
| **Cognito App Client** | `<YOUR_COGNITO_APP_CLIENT_ID>` |
| **API Gateway ID** | `<YOUR_API_GATEWAY_ID>` |
| **Amplify App URL** | `https://main.<YOUR_AMPLIFY_APP_ID>.amplifyapp.com` |
| **S3 バケット（レシート）** | `<YOUR_S3_BUCKET_NAME>` |
...
### 前提条件
* Node.js 18 以上
* AWS アカウント（上記リソースが構築済みであること）

### 手順

1. **リポジトリをクローン**
   ```bash
   git clone [https://github.com/muumin1107/Warikan.git](https://github.com/muumin1107/Warikan.git)
   cd Warikan
   ```
...
3. **環境変数ファイルを作成**
   ...
   ```
   `.env.local` を以下の内容で編集し、ご自身の環境に合わせて `<...>` の部分を書き換えてください。
   ```env
   VITE_API_ENDPOINT=https://<YOUR_API_GATEWAY_ID>[.execute-api.ap-northeast-1.amazonaws.com/dev](https://.execute-api.ap-northeast-1.amazonaws.com/dev)
   VITE_COGNITO_USER_POOL_ID=<YOUR_COGNITO_USER_POOL_ID>
   VITE_COGNITO_CLIENT_ID=<YOUR_COGNITO_APP_CLIENT_ID>
   VITE_COGNITO_DOMAIN=<YOUR_COGNITO_DOMAIN_PREFIX>.auth.ap-northeast-1.amazoncognito.com
...
Amplify Hosting に GitHub リポジトリを連携済みの場合、`main` ブランチへの push で自動デプロイされます。
